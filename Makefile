...
test:		stringsutil
	...
	if test -f test.strings -a "$$(wc -l test.strings 2>/dev/null | awk '{print $$1}')" = 41; then \
		echo "PASS"; \
	else \
	...
	if test -f test.c -a "$$(wc -l test.c 2>/dev/null | awk '{print $$1}')" = 41; then \
		echo "PASS"; \
	else \
	...
	if test -f test.po -a "$$(wc -l test.po 2>/dev/null | awk '{print $$1}')" = 121; then \
		echo "PASS"; \
	else \
	...
	if ./stringsutil -f test.strings import test-zz.po >test.log 2>&1; then \
		if test "$$(wc -l test.strings | awk '{print $$1}')" = 41; then \
			echo "PASS"; \
		else \
	...
	if ./stringsutil -f test.strings import -a test-zz.po >test.log 2>&1; then \
		if test "$$(wc -l test.strings | awk '{print $$1}')" = 43; then \
			echo "PASS"; \
		else \
